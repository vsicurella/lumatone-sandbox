...
DeviceActivityMonitor::DeviceActivityMonitor(TerpstraMidiDriver* midiDriverIn, LumatoneController* controllerIn)
    :   LumatoneSandboxState(*controllerIn, nullptr),
        midiDriver(midiDriverIn), 
        controller(controllerIn),
        readQueueSize(0)
{
    detectDevicesIfDisconnected = controller->getBoolProperty(LumatoneSandboxProperty::DetectDeviceIfDisconnected, true);
    checkConnectionOnInactivity = controller->getBoolProperty(LumatoneSandboxProperty::CheckConnectionIfInactive, true);
    responseTimeoutMs = controller->getIntProperty(LumatoneSandboxProperty::DetectDevicesTimeout, detectRoutineTimeoutMs);
...
void DeviceActivityMonitor::setDetectDeviceIfDisconnected(bool doDetection)
{
    ...
    writeBoolProperty(LumatoneSandboxProperty::DetectDeviceIfDisconnected, detectDevicesIfDisconnected);
...
void DeviceActivityMonitor::setCheckForInactivity(bool monitorActivity)
{
    ...
    writeBoolProperty(LumatoneSandboxProperty::CheckConnectionIfInactive, checkConnectionOnInactivity);
...
{
    juce::String inputId = controller->getStringProperty(LumatoneSandboxProperty::LastInputDeviceId);
    if (inputId.length() <= 0)
        return false;
    ...
    if (inputIndex >= 0)
    {
        juce::String outputId = controller->getStringProperty(LumatoneSandboxProperty::LastOutputDeviceId);
