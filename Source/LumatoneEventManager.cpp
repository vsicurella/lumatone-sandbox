...
LumatoneEventManager::LumatoneEventManager(TerpstraMidiDriver& midiDriverIn, LumatoneController* controllerIn)
    : midiDriver(midiDriverIn), controller(controllerIn)
{
...
{
    auto editMode = controller->getEditorMode();
    if (midiMessage.isSysEx()) switch (editMode)
    {
    ...
    std::function<void(int, void*)> callbackFunctionIfNoError)
{
    ...
    auto errorCode = unpackFunction(midiMessage, boardId, controller->getOctaveSize(), channelData);
...
FirmwareSupport::Error LumatoneEventManager::handleLEDConfigResponse(const juce::MidiMessage& midiMessage)
{
    ...
    if (controller->getConfirmedVersion() < LumatoneFirmwareVersion::VERSION_1_0_11)
    {
        errorCode = midiDriver.unpackGetLEDConfigResponse_Version_1_0_0(midiMessage, boardId, controller->getOctaveSize(), colourData);
    }
    else
    {
...
FirmwareSupport::Error LumatoneEventManager::handleSerialIdentityResponse(const juce::MidiMessage& midiMessage)
{
    ...
    const auto serialNumber = firmwareSupport.serialIdentityToString(serialBytes);
...
//
//void LumatoneEventManager::onConnectionConfirm(bool sendChangeSignal)
//{
//    currentDevicePairConfirmed = true;
//
//    auto lastInputId = midiDriver.getLastMidiInputInfo().identifier;
//    auto lastOutputId = midiDriver.getLastMidiOutputInfo().identifier;
//
//    if (sendChangeSignal)
//        statusListeners.call(&LumatoneEditor::StatusListener::connectionEstablished, lastInputId, lastOutputId);
//}
//
//void LumatoneEventManager::onDisconnection()
//{
...
//
//void LumatoneEventManager::onFirmwareUpdateReceived()
//{
//    juce::jassert(firmwareTransfer != nullptr && editingMode == sysExSendingMode::firmwareUpdate);
//
